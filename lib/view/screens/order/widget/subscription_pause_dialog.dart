...
class SubscriptionPauseDialog extends StatefulWidget {
  ...
  final bool isPause;
  const SubscriptionPauseDialog({Key? key, required this.subscriptionID, required this.isPause}) : super(key: key);
    ...
    if(Get.find<OrderController>().orderCancelReasons != null && Get.find<OrderController>().orderCancelReasons!.isNotEmpty){
      ...
      for(int index=0; index < _reasons.length; index++){
        _cancelReasons.add(DropdownMenuItem<int>(value: index, child: Padding(
          ...
          child: SizedBox(
              height: 30 ,
              child: Center(child: Text(_reasons[index].reason!, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, style: IBMPlexSansArabicMedium.copyWith(fontSize: Dimensions.fontSizeLarge)))),
        ),
        ));
      }
    }
