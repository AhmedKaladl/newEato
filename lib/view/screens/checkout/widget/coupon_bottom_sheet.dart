...
class CouponBottomSheet extends StatelessWidget {
  ...
  final double charge;
  final double total;
  const CouponBottomSheet({Key? key, required this.orderController, required this.price, required this.discount, required this.addOns, required this.deliveryCharge, required this.total, required this.charge}) : super(key: key);
            ...
            children: [
              ...
              Container(
                ...
                child: Row(children: [
                  Expanded(
                    child: SizedBox(
                      ...
                      child: TextField(
                        ...
                        style: IBMPlexSansArabicRegular.copyWith(height: ResponsiveHelper.isMobile(context) ? null : 2),
                        decoration: InputDecoration(
                            hintText: 'enter_promo_code'.tr,
                            hintStyle: IBMPlexSansArabicRegular.copyWith(color: Theme.of(context).hintColor),
                            isDense: true,
                            filled: true,
                  ...
                  InkWell(
                    onTap: () {
                      String couponCode = orderController.couponController.text.trim();
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(children: [
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Text('available_promo'.tr, style: IBMPlexSansArabicBold.copyWith(fontSize: Dimensions.fontSizeDefault)),
                      const SizedBox(),
                    ]),
                    ...
                    couponController.couponList!.isNotEmpty ? GridView.builder(
                        ...
                        return InkWell(
                          onTap: () {
                            if(couponController.couponList![index].code != null) {
                              orderController.couponController.text = couponController.couponList![index].code.toString();
                            }
                            if(orderController.couponController.text.isNotEmpty){
                              if(couponController.discount! < 1 && !couponController.freeDelivery) {
                                if(orderController.couponController.text.isNotEmpty && !couponController.isLoading) {
                                  couponController.applyCoupon(orderController.couponController.text, (price-discount)+addOns, deliveryCharge,
                                    charge, totalPrice, Get.find<RestaurantController>().restaurant!.id, hideBottomSheet: true).then((discount) {

                                    orderController.couponController.text = '${orderController.couponController.text}(${couponController.freeDelivery ? 'free_delivery'.tr : PriceConverter.convertPrice(couponController.discount)})';
                                    if (discount! > 0) {
                                      ...
                                      if(orderController.isPartialPay || orderController.paymentMethodIndex == 1) {
                                        // totalPrice = totalPrice - discount;
                                        orderController.checkBalanceStatus(totalPrice, discount: discount);
                                      }
                                    } else{
                                      if(orderController.isPartialPay || orderController.paymentMethodIndex == 1) {
                                        orderController.checkBalanceStatus(totalPrice);
                      ...
                      children: [
                        ...
                        Text('no_promo_available'.tr, style: IBMPlexSansArabicMedium),
