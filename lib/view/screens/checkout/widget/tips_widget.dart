...
class TipsWidget extends StatelessWidget {
  ...
  final int index;
  const TipsWidget({Key? key, required this.title, required this.isSelected, required this.onTap, required this.isSuggested, required this.index}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(children: [
        ...
        isSuggested ? Text(
          'most_tipped'.tr, style: IBMPlexSansArabicRegular.copyWith(color: Theme.of(context).primaryColor, fontSize: 10),
        ) : const SizedBox(),
      ]),
    );
  }
